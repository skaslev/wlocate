...
static bool is_delimeter(char *delimiters, char c) {
   long i=0,len_delimiters=strlen(delimiters);
   bool is=false;
...
static void parse_parameters(const char *options, int *num_parameters, char ***parameters, char *delimiters) {
  int i=0,j=0,temp=0,num=0, len_options=strlen(options);
  const char *options_temp;
  while  (i<len_options) {
...
//Building the Index//
////////////////////*/
extern void suffixsort(long *x, long *p, long n, long k, long l);

int build_index(uchar *text, ulong length, const char *build_options, void **index){
  char delimiters[] = " =;";
  char filename[256];
  int j,num_parameters;
  char ** parameters;
  long rankb_w=16,rankb_w2=128;
  long free_text=false; /* don't free text by default */
...
///////////////////////*/
int display(void *index, const uchar *pattern, ulong length, ulong numc, ulong *numocc, uchar **snippet_text, ulong **snippet_lengths, ulong **abs_from, ulong **abs_occ) {
  long l,r;
  long pos;
  ulong i, j, from, to, len, x;
  uchar *text_aux;
  CSA *SA=(CSA *) index;
  csa_bsearch(pattern,length,SA,&l,&r);
  *numocc = r-l+1;
  *abs_occ = csa_batchlookup2(SA,l,r);
  *snippet_lengths = (ulong *) malloc((*numocc)*sizeof(ulong));
  if (!(*snippet_lengths)) return 1;
  *abs_from = (ulong *) malloc((*numocc)*sizeof(ulong));
  if (!(*abs_from)) return 1;
